...
void TextCtrl::activateAnchor()
{
    if( view()->getCursor().isUrl() )
        emit anchorActivated( view()->getCursor().getUrlEncoded(), true );
    else if( view()->getCursor().isLink() )
    {
        const QByteArray link = view()->getCursor().getLink();
        emit anchorActivated( link, false );
        emit linkActivated( link ); // obsolet
    }
}
...
bool TextCtrl::keyPressEvent(QKeyEvent *e)
{
    ...
    if( e == QKeySequence::Paste && QApplication::clipboard()->mimeData()->hasText() )
    {
        view()->getCursor().insertText( QApplication::clipboard()->mimeData()->text().simplified() );
        goto accept;
    }
    if( e == QKeySequence::Cut )
    {
        view()->copy();
        view()->getCursor().removeSelectedText();
        goto accept;
    }
