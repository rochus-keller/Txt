...
TextInStream::TextInStream( const Styles* s, const LinkRendererInterface *l ):d_linkRenderer(l)
{
	if( s == 0 )
			...
			case ReadAnch:
				if( t == DataReader::Slot )
				{
					...
					if( name.equals( "url" ) )
						...
						anchorText = v.toString();
					else
						throw _MyException( "invalid slot" );
				}else if( t == DataReader::EndFrame )
				{
					//qDebug() << url << urlText; // TEST
                    if( link.isEmpty() && url.startsWith( Styles::s_linkSchema ) )
                        link = QByteArray::fromBase64( url.mid( ::strlen( Styles::s_linkSchema ) ) );
                    if( !link.isEmpty() )
                    {
                        if( d_linkRenderer != 0 && !d_linkRenderer->renderLink( cur, link ) )
						{
                            //Text dynamisch berechnen, ansonsten gespeicherter Text
							if( d_linkRenderer )
								cur.insertUrl( d_linkRenderer->renderHref(link), true, anchorText );
							else
								cur.insertUrl( LinkRendererInterface::defaultRenderHref(link), true, anchorText );
						}
                    }else
                        cur.insertUrl( url, true, anchorText );
					state = oldBlockState;
				}else
					throw _MyException( "invalid token" );
				break;
